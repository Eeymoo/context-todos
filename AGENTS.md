...
### Database
- SQLite via libsql, stored under `os.tmpdir()/context-todos/<sha256-of-watchPath>/context-todos.db`, so each watch path shares the same `/tmp/context-todos` root but gets its own hashed subfolder
